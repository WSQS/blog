...
通过只存在一份非终止符来合并重复的子树。这将树变成了有向无环图。

这可以让无限歧义的parsing用有限的数据结构表示，会成为一个parse图。。

##### Retrieving Parse Trees from a Parse Forest

对接收者来说，可以生成也可以对数据结构进行剪枝。

对生成Parse树，可以简单的使用深度优先搜索进行生成。

减枝可以基于节点的信息和类型冲突与非来判断，比如通过结合律来判断。

#### Parse-Forest Grammars

将Parsing结果作为语法是奇怪的。但是Parse森林语法是有一些优点的。

使用A_i_l表示非终止符A，生成了从i开始长度为l的符号。

优点在于，明确表述了对给定的子串，只存在一个非终止符。这在数学上也是优美的，parsing是一个函数，将语法映射成语法，用于处理语法的函数也可以处理Parse森林语法。同时，Parse森林语法是可以简单的清理的，只需要移除规则即可。表示无限歧义语法也是简单的。契合将解析视为交集。

Parse森林和Parse森林语法是实际上一致的。只是指针和名称的区别，名称比指针更加强大。并且与-或树机制是parse森林语法当中自带的。

### When are we done Parsing?

对无向parser，判断Parsing结束是容易的。

对有向parser，情况变得复杂。

- Parser是否到打输入的末端，他是否完全消费输入的最后一个token？
- Parser处于接受状态吗？
- Parser能否继续，是否有下一个token并且parser能否处理它？
- Parser是否用于生成Parse树或者只是用于进行判断？
- 是想要所有的Parsing树还是只需要一个？
- Parser接受了整个输入还是只是识别最长合法前缀？

尾随垃圾问题。

输入结束符号。这样唯一的接受状态是接受首个输入结束符号。

### Transitive Closure

许多parsing算法以一些初始信息开始随后通过推断规则得到结论。用闭包算法来获得结论。

传递性。

闭包算法的效率取决于推断规则。传递性闭包有3种方法：简单的，传统的和先进的。简单方法通常比较高效，但是需要大量轮数才能覆盖比较大的图，会重复计算旧的结果若干次。

传统算法使用warshill算法，它的优点在于它非常容易实现并且时间复杂度只依赖于节点数量而不是边的数量。缺点在于它的时间复杂度是节点数量的3次方。

先进算法避免了简单算法的低效。循环被构建为强连接组件，边以一种避免重复结论的方式组合，并且允许边的集合进行拷贝而不是重新计算，使用了高效的表达方式。它的时间复杂度与结论的数量保持线性关系。

先进的传递闭包算法是在很多场景下有用，但是在persing中这是可疑的。

算法的闭包性可以让我们关注于推导规则而忽略依赖的闭包算法。

### The Relation between Parsing and Boolean Matrix Multiplication

Parsing和布尔矩阵操作之间的关联。两者之间可以相互转换。

矩阵乘法。

闭包算法其实就是无限次矩阵乘法。

ref: Algebraic complexity theory V. Strassen

上下文无关parsing问题可以被转换为一个布尔矩阵操作问题。这目前只是一个理论方向，并不实际。

布尔矩阵操作问题也可以被变换为一个上下文无关parsing问题。这是一个有价值的方向，但是目前还没有发现这样的算法。

通用上下文无关parsing问题和格中的最短路径计算的相似性。

最困难的上下文无关语言。

### Conclusion

数据驱动parsing，无语法的上下文无关parsing，适合处理自然语言。

### Problems

> Suppose all terminal symbols in a given grammar are different. Is that grammar unambiguous?

不是的，所有终止符都是不同的只能保证终止符可以无歧义的转换为一个非终止符，但是歧义性结构是可以在非终止符中展开的。

> Write a program that, given a grammar G and a number n, computes the number of different parse trees with n leaves (terminals) G allows.

> If you are familiar with an existing parser (generator), identify its parser components, as described on page 69.

> The maze preprocessing algorithm in Section 3.5.4 eliminates all rooms with three walls; rules with two or four walls are acceptable in a deterministic maze. What about rooms with zero or one wall? How do they affect the algorithm and the result? Is it possible/useful to eliminate them too?

对于单一的0墙和1墙房间，其实是不应该去除的，因为会存在一个房间是十字路口或者丁字路口的情况。所以关键在于与周围房间的联系，如果增加一个墙不会改变周围四个房间的联通关系，那么就可以增加，否则则不行。当四条边都是可以增加的状态，那么这个房间是可以去除的。
不过这样的方式会修改可能的解，所以这样通常来说是不行的。
对于效果来说这样的方式可以减少检索的一个搜索选项，可以提升一点性能。

> Construct an example in which a deterministic bottom-up parser will have to perform k actions at position k, for a certain k.
...
迷宫和语法的映射分析，迷宫的每一个格子是一个非终止符，出口是接受状态也就是$\epsilon$，空间之间的连接关系也就是符号之间的转换关系。
对迷宫的表示：格子、出口之间的连接关系。
迷宫的入口是启动符号。
可以直接将这样的连接关系转换为语法的规则，每两个相邻可以通行的格子之间都可以定义一个转换规则，每个规则右侧为一个与两个格子之间的有向关联一一对应的终止符和新的格子的非终止符。这样最终生成的语句就是一个路径。

> Project: Study the “translate and cross out matching parentheses” parsing method of Greibach [389].

> Show that a version of Figure 3.14 in which the nodes marked 2 near the top are combined represents parse trees that are not supported by the input.

> Implement the backtracking algorithm sketched in Section 3.7.3.

> Research project: Some parsing problems involve extremely large CF grammar, with millions of rules. Such a grammar is generated by program and results from incorporating finite context conditions into the grammar. It is usually very redundant, containing many very similar rules, and very ambiguous. Many general CF parsers are quadratic in the size of the grammar, which for ten million rules brings in a factor of 1014. Can parsing techniques be designed that work well on such grammars? (See also Problem 4.5.)

## General Non-Directional Parsing

两种非定向Parsing方法：Unger和CYK。

Unger方法是自顶向下的。对每个启动符号的生成式，尝试递归的设定子串来进行匹配。

CYK方法是自底向上的。从最终的句子开始尝试，用所有的规则反向生成所有可能的句子。

### Unger’s Parsing Method

#### Unger’s Method without $\epsilon$-Rules or Loops

首先需要生成所有分区，可以采用递归的方式。

对于搜索，Unger采用深度优先的方法。

即使是简单的例子需要相当量的工作。但同时一些简单的检查就可以节约大量的工作。

#### Unger’s Method with $\epsilon$-Rules
...
记录已经访问过的问题列表。在要进行新的问题时，对问题列表进行检查，查看是否存在重复的内容，如果存在则跳过这个问题。

一个优化的方向是对问题答案进行缓存。

#### Getting Parse-Forest Grammars from Unger Parsing

在Unger parsing时，构建parse森林是很简单的。只需要给parse森林语法给每一个区分规则增加规则。

名称是开始位置加上长度，这对起始符号也有效。

Unger parsing创建了大量未定义的非终止符。

### The CYK Parsing Method

CYK算法的两个阶段，识别阶段，构建一个表说明哪个非终止符推导出句子的哪部分子串，随后使用识别表和语法来构建所有可能的句子的推导。

#### CYK Recognition with General CF Grammars

CYK算法首先关注输入句子的子串，越短的子串越优先。

原子规则：A -> B，A和B都是非终止符。

原子规则会让复杂度上升。

$\epsilon$规则也会让复杂度上升。任意两个相邻的非终止符中都可以有一个$\epsilon$符号。

因为CYK算法中越短的子串越优先，所以最先的子串就是$\epsilon$子串。计算$R_\epsilon$，生成$\epsilon$的非终止符的集合。

$R_\epsilon$可以通过闭包算法得到。

对子串的表示:$s_{i,l}$

$R_{i,l}$表示生成$s_{i,l}$的非终止符集。

当对长度为l的子串进行处理时，假定所有长度小于l的子串的信息都是已知的。对每个规则的右侧进行检查，并将子串区分成m个部分。与Unger方法不同的是，这里不需要尝试所有的分区，因为已经知道哪些非终止符推断哪些子串。

存在两个问题，原子规则可以用闭包算法解决。$\epsilon$也可以用闭包算法解决。

另一个严重的问题在于规则右侧可能包含任意多数数量的非终止符，尝试所有的可能性是较为大的工作。时间复杂度对规则右侧长度来说是指数的。

假定一些对规则的约束可以很大程度上解决这个问题。

#### CYK Recognition with a Grammar in Chomsky Normal Form

两个规则约束：无原子规则和$\epsilon$规则。限制规则右侧最大长度。乔姆斯基范式(Chomsky Normal Form, CNF)正好符合这些要求。所有上下文无关语法都可以机械地转换为一个CNF语法。

使用CNF语法的CYK方法十分简单，不再需要考虑闭包，同时划分也只需要考虑两部分的情况。

$R_{i,l}$可以被组织成一个三角表，也就是识别表。

计算识别表的顺序。可以以子串的长度，也可以基于整个串最后一个字符的长度。后者可以比较适合文本逐渐出现的情况。

最坏情况的时间复杂度和句子长度的三次方成正比。

时间复杂度还和语法有关系，和非终止符的数量有关系。

索引使用长度还是终止点其实是任意的，可以怎么方便怎么来。

#### Transforming a CF Grammar into Chomsky Normal Form

将上下文无关文法转换为乔姆斯基范式的步骤：
...
参见 Hopcroft 和 Hllman 的对变换不会改变语法的证明。

##### Eliminating $\epsilon$-Rules

对每一个可能为空的非终止符，需要生成新的规则，将规则右侧的该终止符去除。并且将该非终止符的$\epsilon$规则去除。
...
##### Finally, to the Chomsky Normal Form

使用替换来进行转换。

#### The Example Revisited

存在两种计算方式，一种是先检查所有规则，再看是否可以匹配上，另一种是根据目前的识别表计算可能的右侧。

#### CYK Parsing with Chomsky Normal Form

基于识别表，需要获得句子的Parsing信息。

通过识别表获得的生成结构是转换过的乔姆斯基范式的，而不是原始的。

#### Undoing the Effect of the CNF Transformation

添加被移除的非终止符。同时也需要添加一行来标记$\epsilon$规则非终止符。

根据原本的规则，再次逐规则按顺序进行判断。和Unger算法类似。

*可以尝试标记规则之间的映射关系，并将其进行还原来完成这部分工作。*

#### A Short Retrospective of CYK

#### Getting Parse-Forest Grammars from CYK Parsing

Parse森林语法比识别表拥有更多的信息。

...
文章的组织方式是以范畴的方式进行，范畴，又或者说视角，我认为是处理对 AI 的观点时一个重要的因素，目前对于 AI 问题众说纷纭的一个原因就是很多观点实际上视角不同。

举例来说，在LLM基础能力这个层面，随着架构和数据等的升级，模型的能力会逐步上升，但是这样的能力提升并不意味着应用层的问题就能够随之解决，在[CoT(Chain-of-Thought)][2]中就发现，尽管随着模型的发展它的能力逐渐提升，但是还是需要通过在 Prompt 当中进行引导才可以让模型涌现出推理的能力。
...
思维链([CoT][2])尝试通过引入中间推理步骤来提升大模型执行复杂推理的能力。这里存在两种核心观点，一个是添加自然语言推理步骤来引导模型进行推理，另一个是对新任务，并不需要对模型进行训练或微调，只需要在上下文中提供一些例子，大模型就可以完成这个任务，也就是`few-shot prompting`。思维链的核心是结合了这两个观点，由前者提供内容，后者提供形式。
